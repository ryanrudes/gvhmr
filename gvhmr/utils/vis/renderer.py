...
# pytorch3d is the optional 'render' extra (CUDA/Linux mesh rasterization). Keep this
# module importable without it — the pure-numpy helpers below and downstream datasets
# only need it when a `Renderer` is actually constructed (guarded in __init__).
try:
    ...
    _HAS_PYTORCH3D = True
except ImportError:  # pragma: no cover - exercised on installs without the render extra
    _HAS_PYTORCH3D = False

from gvhmr.utils._optional import missing_dependency
...
class Renderer:
    def __init__(self, width, height, focal_length=None, device="cuda", faces=None, K=None, bin_size=None):
        ...
        if not _HAS_PYTORCH3D:
            raise missing_dependency("Mesh rendering", package="pytorch3d", extra="render")
        self.width = width
        self.height = height
...
def get_global_cameras_static(
    verts, beta=4.0, cam_height_degree=30, target_center_height=1.0, use_long_axis=False, vec_rot=45, device="cpu"
):
    L, V, _ = verts.shape
