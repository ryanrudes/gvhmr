...
def quat_norm_check(x):
    ...
    assert bool((abs(x.norm(p=2, dim=-1) - 1) < 1e-3).all()), (
        f"the quaternion is has non-1 norm: {abs(x.norm(p=2, dim=-1) - 1)}"
    )
...
def forward_kinematics(mat, parent):
    ...
    if isinstance(mat, torch.Tensor):
        ...
        # The torch.cat path below rebuilds the whole (..., N, d, d) tensor for every
        # joint (O(N^2) allocation) purely so autograd can track it. When grad isn't
        # needed (inference) in-place assembly is O(N) and *numerically identical*.
        track_grad = torch.is_grad_enabled() and mat.requires_grad
        if not track_grad:
