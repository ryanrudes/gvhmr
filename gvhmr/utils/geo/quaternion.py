...
def qeuler(q: torch.Tensor, order: str, epsilon: float = 0, deg: bool = True) -> torch.Tensor:
    """
...
def qeuler_np(q: np.ndarray, order: str, epsilon: float = 0, use_gpu: bool = False) -> np.ndarray:
    if use_gpu:
...
def euler2quat(e: torch.Tensor, order: str, deg: bool = True) -> torch.Tensor:
    """
...
def expmap_to_quaternion(e: np.ndarray) -> np.ndarray:
    """
...
def euler_to_quaternion(e: np.ndarray, order: str) -> np.ndarray:
    """
    Convert Euler angles to quaternions.
    """
    assert e.shape[-1] == 3
...
def qpow(q0, t, dtype=torch.float):
    ...
    """
    q0 = qnormalize(q0)
    # Clamp to acos's domain: after normalization the real part is in [-1, 1] on CPU,
    # but float error on some backends (e.g. MPS) can nudge it just past 1.0, which makes
    # acos return NaN. The clamp is a no-op for in-domain inputs.
    theta0 = torch.acos(q0[..., :1].clamp(-1.0, 1.0))
...
def qslerp(q0: torch.Tensor, q1: torch.Tensor, t: torch.Tensor | float) -> torch.Tensor:
    """
...
def lerp(p0: torch.Tensor, p1: torch.Tensor, t: torch.Tensor | float) -> torch.Tensor:
