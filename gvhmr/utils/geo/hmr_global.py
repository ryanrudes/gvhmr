...
from gvhmr.utils.net_utils import gaussian_smooth


def get_R_c2gv(R_w2c: torch.Tensor, axis_gravity_in_w: list[int] | torch.Tensor = [0, 0, -1]) -> torch.Tensor:
    """
    Args:
        R_w2c: (*, 3, 3)
    Returns:
...
def get_tgtcoord_rootparam(
    ...
    transl: torch.Tensor,
    gravity_vec: torch.Tensor | None = None,
    tgt_gravity_vec: torch.Tensor | None = None,
    tsf: str = "ay->ay",
...
def get_c_rootparam(
    global_orient: torch.Tensor, transl: torch.Tensor, T_w2c: torch.Tensor, offset: torch.Tensor
...
def get_T_w2c_from_wcparams(
    global_orient_w: torch.Tensor,
    transl_w: torch.Tensor,
    global_orient_c: torch.Tensor,
    transl_c: torch.Tensor,
    offset: torch.Tensor,
) -> torch.Tensor:
    """
    Args:
...
def get_local_transl_vel(transl: torch.Tensor, global_orient: torch.Tensor) -> torch.Tensor:
    """
...
def get_local_transl_vel_alignhead(transl: torch.Tensor, global_orient: torch.Tensor) -> torch.Tensor:
...
def rollout_local_transl_vel_alignhead(
    local_transl_vel_alignhead: torch.Tensor, global_orient: torch.Tensor, transl_0: torch.Tensor | None = None
) -> torch.Tensor:
    # assume global_orient is ay
    global_orient_rot = axis_angle_to_matrix(global_orient)  # (*, 3, 3)
    global_orient_quat = matrix_to_quaternion(global_orient_rot)  # (*, 4)
...
def get_local_transl_vel_alignhead_absy(transl: torch.Tensor, global_orient: torch.Tensor) -> torch.Tensor:
    # assume global_orient is ay
    global_orient_rot = axis_angle_to_matrix(global_orient)  # (*, 3, 3)
    global_orient_quat = matrix_to_quaternion(global_orient_rot)  # (*, 4)
...
def rollout_local_transl_vel_alignhead_absy(
    local_transl_vel_alignhead_absy: torch.Tensor, global_orient: torch.Tensor, transl_0: torch.Tensor | None = None
) -> torch.Tensor:
    # assume global_orient is ay
    global_orient_rot = axis_angle_to_matrix(global_orient)  # (*, 3, 3)
    global_orient_quat = matrix_to_quaternion(global_orient_rot)  # (*, 4)
...
def get_local_transl_vel_alignhead_absgy(transl: torch.Tensor, global_orient: torch.Tensor) -> torch.Tensor:
    # assume global_orient is ay
    global_orient_rot = axis_angle_to_matrix(global_orient)  # (*, 3, 3)
    global_orient_quat = matrix_to_quaternion(global_orient_rot)  # (*, 4)
...
def rollout_local_transl_vel_alignhead_absgy(
    local_transl_vel_alignhead_absgy: torch.Tensor, global_orient: torch.Tensor, transl_0: torch.Tensor | None = None
) -> torch.Tensor:
    # assume global_orient is ay
    global_orient_rot = axis_angle_to_matrix(global_orient)  # (*, 3, 3)
    global_orient_quat = matrix_to_quaternion(global_orient_rot)  # (*, 4)
...
def rollout_vel(vel: torch.Tensor, transl_0: torch.Tensor | None = None) -> torch.Tensor:
    """
    Args:
        vel: (*, L, 3)
        transl_0: (*, 1, 3), if not provided, the start point is 0
...
def get_static_joint_mask(
    w_j3d: torch.Tensor, vel_thr: float = 0.25, smooth: bool = False, repeat_last: bool = False
) -> torch.Tensor:
    """
    w_j3d: (*, L, J, 3)
