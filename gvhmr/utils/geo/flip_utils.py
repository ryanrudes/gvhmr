...
def flip_heatmap_coco17(output_flipped: torch.Tensor) -> torch.Tensor:
...
def flip_bbx_xys(bbx_xys: torch.Tensor, w: int) -> torch.Tensor:
    """
    bbx_xys: (F, 3)
    """
...
def flip_kp2d_coco17(kp2d: torch.Tensor, w: int) -> torch.Tensor:
    """Flip keypoints."""
    kp2d = kp2d.clone()
...
def flip_smplx_params(smplx_params: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    """Flip pose.
...
def avg_smplx_aa(aa1: torch.Tensor, aa2: torch.Tensor) -> torch.Tensor:
    def avg_rot(rot: torch.Tensor) -> torch.Tensor:
        # input [B,...,3,3] --> output [...,3,3]
        rot = rot.mean(dim=0)
        # torch.linalg.svd returns Vh (= V^T), so U @ V.transpose(-1,-2) == U @ Vh.
        U, _, Vh = torch.linalg.svd(rot, full_matrices=False)
        rot = U @ Vh
        return rot
