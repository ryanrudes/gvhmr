...
from gvhmr.utils.geo_transform import convert_bbx_xys_to_lurb, cvt_to_bi01_p2d


def estimate_focal_length(img_w: float, img_h: float) -> float:
    return (img_w**2 + img_h**2) ** 0.5  # Diagonal FOV = 2*arctan(0.5) * 180/pi = 53


def estimate_K(img_w: float, img_h: float) -> torch.Tensor:
...
def convert_K_to_K4(K: torch.Tensor) -> torch.Tensor:
    K4 = torch.stack([K[0, 0], K[1, 1], K[0, 2], K[1, 2]]).float()
    return K4


def convert_f_to_K(focal_length: float, img_w: float, img_h: float) -> torch.Tensor:
    K = torch.eye(3).float()
    K[0, 0] = focal_length
    K[1, 1] = focal_length
    K[0, 2] = img_w / 2.0
...
def resize_K(K: torch.Tensor, f: float = 0.5) -> torch.Tensor:
    K = K.clone() * f
    K[..., 2, 2] = 1.0
    return K
...
def create_camera_sensor(
    width: int | None = None, height: int | None = None, f_fullframe: float | None = None
...
def convert_xys_to_cliff_cam_wham(xys: torch.Tensor, res: torch.Tensor) -> torch.Tensor:
    """
    Args:
    ...
    def normalize_keypoints_to_image(x: torch.Tensor, res: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: (N, 2), centers
            res: (2), e.g. [4112., 3008.]
...
def get_a_pred_cam(transl: torch.Tensor, bbx_xys: torch.Tensor, K_fullimg: torch.Tensor) -> torch.Tensor:
...
def project_to_bi01(points: torch.Tensor, bbx_xys: torch.Tensor, K_fullimg: torch.Tensor) -> torch.Tensor:
    """
    points: (B, L, J, 3)
    bbx_xys: (B, L, 3)
    K_fullimg: (B, L, 3, 3)
...
def perspective_projection(points: torch.Tensor, K: torch.Tensor) -> torch.Tensor:
    # points: (B, L, J, 3)
    # K: (B, L, 3, 3)
...
def get_bbx_xys(
    i_j2d: torch.Tensor,
    bbx_ratio: list[int] = [192, 256],
    do_augment: bool = False,
    base_enlarge: float = 1.2,
) -> torch.Tensor:
...
def safely_render_x3d_K(x3d: torch.Tensor, K_fullimg: torch.Tensor, thr: float) -> torch.Tensor:
    """
    Args:
    ...
    if (x3d_unsafe_mask).sum() > 0:
        ...
        if False:
            from gvhmr.utils.wis3d_utils import make_wis3d
...
def get_bbx_xys_from_xyxy(bbx_xyxy: torch.Tensor, base_enlarge: float = 1.2) -> torch.Tensor:
    """
    Args:
...
def bbx_xyxy_from_x(p2d: torch.Tensor) -> torch.Tensor:
    """
    Args:
...
def bbx_xyxy_from_masked_x(p2d: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    """
    Args:
        p2d: (*, V, 2) - Tensor containing 2D points.
...
def bbx_xyxy_ratio(xyxy1: torch.Tensor, xyxy2: torch.Tensor) -> torch.Tensor:
...
def get_mesh_in_fov_category(mask: torch.Tensor) -> tuple[int, torch.Tensor]:
    """mask: (L, V)
    The definition:
...
def get_infov_mask(p2d: torch.Tensor, w_real: torch.Tensor | int, h_real: torch.Tensor | int) -> torch.Tensor:
    """
    Args:
        p2d: (B, L, V, 2)
