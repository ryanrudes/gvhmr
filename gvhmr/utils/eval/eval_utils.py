...
@torch.no_grad()
def compute_global_metrics(batch: dict, mask: torch.Tensor | None = None) -> dict[str, np.ndarray]:
...
@torch.no_grad()
def compute_camcoord_perjoint_metrics(batch: dict, pelvis_idxs: list[int] = [1, 2]) -> dict[str, np.ndarray]:
    """
    Args:
        batch (dict): {
            "pred_j3d": (..., J, 3) tensor
...
def compute_jpe(S1: torch.Tensor, S2: torch.Tensor) -> np.ndarray:
    return torch.sqrt(((S1 - S2) ** 2).sum(dim=-1)).mean(dim=-1).numpy()


def compute_perjoint_jpe(S1: torch.Tensor, S2: torch.Tensor) -> np.ndarray:
    return torch.sqrt(((S1 - S2) ** 2).sum(dim=-1)).numpy()


def batch_align_by_pelvis(
    data_list: list[torch.Tensor], pelvis_idxs: list[int] = [1, 2]
...
def batch_compute_similarity_transform_torch(S1: torch.Tensor, S2: torch.Tensor) -> torch.Tensor:
    """
...
def compute_rte(target_trans: torch.Tensor, pred_trans: torch.Tensor) -> np.ndarray:
...
def compute_jitter(joints: torch.Tensor, fps: float = 30) -> np.ndarray:
...
def compute_foot_sliding(target_verts: torch.Tensor, pred_verts: torch.Tensor, thr: float = 1e-2) -> np.ndarray:
...
def convert_joints22_to_24(
    joints22: torch.Tensor, ratio2220: float = 0.3438, ratio2321: float = 0.3345
) -> torch.Tensor:
...
def align_pcl(
    Y: torch.Tensor, X: torch.Tensor, weight: torch.Tensor | None = None, fixed_scale: bool = False
...
def global_align_joints(gt_joints: torch.Tensor, pred_joints: torch.Tensor) -> torch.Tensor:
    """
...
def first_align_joints(gt_joints: torch.Tensor, pred_joints: torch.Tensor) -> torch.Tensor:
    """
...
def rearrange_by_mask(x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    """
    x (L, *)
    mask (M,), M >= L
    """
...
def as_np_array(d: torch.Tensor | np.ndarray | object) -> np.ndarray:
