...
# From GPT
def get_frame_id_list_from_mask(mask: torch.Tensor) -> list[torch.Tensor]:
    # batch=64, 0.13s
    """
...
def get_batch_frame_id_lists_from_mask_BLC(masks: torch.Tensor) -> list[list[list[torch.Tensor]]]:
    # batch=64, 0.10s
    """
...
def get_frame_id_list_from_frame_id(frame_id: torch.Tensor) -> list[torch.Tensor]:
...
def frame_id_to_mask(frame_id: torch.Tensor, max_len: int) -> torch.Tensor:
    mask = torch.zeros(max_len, dtype=torch.bool)
    mask[frame_id] = True
    return mask


def mask_to_frame_id(mask: torch.Tensor) -> torch.Tensor:
    frame_id = torch.where(mask)[0]
    return frame_id


def linear_interpolate_frame_ids(data: torch.Tensor, frame_id_list: list[torch.Tensor]) -> torch.Tensor:
    data = data.clone()
...
def linear_interpolate(data: torch.Tensor, N_middle_frames: int) -> torch.Tensor:
    """
    Args:
        data: (2, C)
    Returns:
...
def find_top_k_span(mask: torch.Tensor | np.ndarray, k: int = 3) -> list[tuple[int, int]]:
    """
    Args:
        mask: (L,)
    Return:
