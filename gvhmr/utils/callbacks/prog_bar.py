...
class ProgressReporter(ProgressBar, pl.Callback):
    ...
    def get_metrics(self, trainer: pl.Trainer, pl_module: pl.LightningModule) -> dict[str, str | float]:
...
class EmojiProgressReporter(ProgressBar, pl.Callback):
    def __init__(
        self,
        refresh_rate_batch: int | None = 1,  # report interval of batch, set None to disable it
