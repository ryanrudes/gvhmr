...
import gvhmr.utils.matrix as matrix
from gvhmr.utils.geo.quaternion import qbetween
from gvhmr.utils.geo.rotations import (
...
def homo_points(points: torch.Tensor) -> torch.Tensor:
    """
    Args:
        points: (..., C)
...
def apply_Ts_on_seq_points(points: torch.Tensor, Ts: torch.Tensor) -> torch.Tensor:
    """
...
def T_transforms_points(T: torch.Tensor, points: torch.Tensor, pattern: str) -> torch.Tensor:
...
def project_p2d(points: torch.Tensor, K: torch.Tensor | None = None, is_pinhole: bool = True) -> torch.Tensor:
    """
    Args:
        points: (..., (N), 3)
        K: (..., 3, 3)
...
def gen_uv_from_HW(H: int, W: int, device: str = "cpu") -> torch.Tensor:
...
def unproject_p2d(uv: torch.Tensor, z: torch.Tensor, K: torch.Tensor) -> torch.Tensor:
...
def cvt_p2d_from_i_to_c(uv: torch.Tensor, K: torch.Tensor) -> torch.Tensor:
    """
    Args:
        uv: (..., 2) or (..., N, 2)
        K: (..., 3, 3)
...
def cvt_to_bi01_p2d(p2d: torch.Tensor, bbx_lurb: torch.Tensor) -> torch.Tensor:
    """
    p2d: (..., (N), 2)
    bbx_lurb: (..., 4)
    """
...
def cvt_from_bi01_p2d(bi01_p2d: torch.Tensor, bbx_lurb: torch.Tensor) -> torch.Tensor:
...
def cvt_p2d_from_bi01_to_c(bi01: torch.Tensor, bbxs_lurb: torch.Tensor, Ks: torch.Tensor) -> torch.Tensor:
    """
    Args:
...
def cvt_p2d_from_pm1_to_i(p2d_pm1: torch.Tensor, bbx_xys: torch.Tensor) -> torch.Tensor:
    """
    Args:
...
def uv2l_index(uv: torch.Tensor, W: int) -> torch.Tensor:
    return uv[..., 0] + uv[..., 1] * W


def l2uv_index(l: torch.Tensor, W: int) -> torch.Tensor:
...
def axis_angle_to_matrix_exp_map(aa: torch.Tensor) -> torch.Tensor:
...
def matrix_to_axis_angle_log_map(R: torch.Tensor) -> torch.Tensor:
...
def ransac_PnP(K: np.ndarray, pts_2d: np.ndarray, pts_3d: np.ndarray, err_thr: float = 10) -> tuple:
    """solve pnp"""
...
def ransac_PnP_batch(
    K_raw: np.ndarray, pts_2d: np.ndarray, pts_3d: np.ndarray, err_thr: float = 10
...
def triangulate_point(Ts_w2c: torch.Tensor, c_p2d: torch.Tensor, **kwargs):
    from gvhmr.utils.geo.triangulation import triangulate_persp

    Log.warning("Deprecated, please import from gvhmr.utils.geo.triangulation")
    return triangulate_persp(Ts_w2c, c_p2d, **kwargs)


def triangulate_point_ortho(Ts_w2c: torch.Tensor, c_p2d: torch.Tensor, **kwargs):
    from gvhmr.utils.geo.triangulation import triangulate_ortho
...
def get_nearby_points(
    points: torch.Tensor, query_verts: torch.Tensor, padding: float = 0.0, p: int = 1
) -> torch.Tensor:
    """
    points: (S, 3)
    query_verts: (V, 3)
    ...
    if p == 1:
        ...
        # Lazy import: pytorch3d's KNN is a compiled op (CUDA/Linux). Only this
        # p==2 branch needs it, so the module imports fine without pytorch3d.
        try:
            import pytorch3d.ops.knn as knn
        except ImportError as e:  # pragma: no cover - exercised only without pytorch3d
            raise ImportError(
                "get_nearby_points(p=2) needs pytorch3d's KNN op. Install the 'render' "
                "extra / pytorch3d, or use p=1 (axis-aligned bbox) instead."
            ) from e
...
def unproj_bbx_to_fst(
    bbx_lurb: torch.Tensor,
    K: torch.Tensor,
    near_z: float | torch.Tensor = 0.5,
    far_z: float | torch.Tensor = 12.5,
) -> torch.Tensor:
    B = bbx_lurb.size(0)
...
def convert_bbx_xys_to_lurb(bbx_xys: torch.Tensor) -> torch.Tensor:
    """
...
def convert_lurb_to_bbx_xys(bbx_lurb: torch.Tensor) -> torch.Tensor:
    """
...
def compute_T_ayf2az(joints: torch.Tensor, inverse: bool = False) -> torch.Tensor:
    """
    Args:
...
def compute_T_ay2ayrot(joints: torch.Tensor) -> torch.Tensor:
    """
    Args:
        joints: (B, J, 3), in the start-frame, ay-coordinate
    Returns:
...
def compute_root_quaternion_ay(joints: torch.Tensor) -> torch.Tensor:
    """
    Args:
        joints: (B, J, 3), in the start-frame, ay-coordinate
    Returns:
...
def similarity_transform_batch(
    ...
    # torch.linalg.svd returns Vh (= V^T); transpose back to V to preserve the
    # original torch.svd semantics (otherwise the recovered rotation inverts).
...
def kabsch_algorithm_batch(X1: torch.Tensor, X2: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """
...
def compute_cam_angvel(R_w2c: torch.Tensor, padding_last: bool = True) -> torch.Tensor:
    """
    R_w2c : (F, 3, 3)
    """
...
def ransac_gravity_vec(
    xyz: torch.Tensor, num_iterations: int = 100, threshold: float = 0.05, verbose: bool = False
...
def sequence_best_cammat(
    w_j3d: torch.Tensor, c_j3d: torch.Tensor, cam_rot: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor]:
...
def get_sequence_cammat(w_j3d: torch.Tensor, c_j3d: torch.Tensor, cam_rot: torch.Tensor) -> torch.Tensor:
    # w_j3d: (L, J, 3)
    # c_j3d: (L, J, 3)
    # cam_rot: (L, 3, 3)
...
def ransac_vec(vel: torch.Tensor, min_multiply: float = 20, verbose: bool = False) -> tuple[torch.Tensor, torch.Tensor]:
    # xyz: (L, 3)
