...
from gvhmr.model.gvhmr.utils import stats_compose
from gvhmr.model.gvhmr.utils.endecoder import EnDecoder
from gvhmr.model.gvhmr.utils.postprocess import (
    pp_static_joint,
    pp_static_joint_cam,
    process_ik,
)
from gvhmr.utils.geo.hmr_cam import compute_bbox_info_bedlam, compute_transl_full_cam, get_a_pred_cam, project_to_bi01
from gvhmr.utils.geo.hmr_global import (
...
class Pipeline(nn.Module):
    def __init__(self, args, args_denoiser3d, **kwargs) -> None:
        super().__init__()
        self.args = args
...
def randomly_set_null_condition(f_condition: dict, uncond_prob: float = 0.1) -> dict:
...
def compute_extra_incam_loss(inputs: dict, outputs: dict, ppl: Pipeline) -> tuple[torch.Tensor | int, dict]:
...
def compute_extra_global_loss(inputs: dict, outputs: dict, ppl: Pipeline) -> tuple[torch.Tensor | int, dict]:
    decode_dict = outputs["decode_dict"]
    endecoder = ppl.endecoder
    weights = ppl.weights
    args = ppl.args
...
def get_smpl_params_w_Rt_v2(
    global_orient_gv: torch.Tensor,
    local_transl_vel: torch.Tensor,
    ...
    def as_identity(R: torch.Tensor) -> torch.Tensor:
