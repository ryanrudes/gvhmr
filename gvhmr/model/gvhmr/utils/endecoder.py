...
from gvhmr.utils.geo.augment_noisy_pose import gaussian_augment
from gvhmr.utils.geo.hmr_global import get_local_transl_vel, rollout_local_transl_vel
...
class EnDecoder(nn.Module):
    def __init__(self, stats_name: str = "DEFAULT_01", noise_pose_k: int = 10) -> None:
        super().__init__()
        # Load mean, std
    ...
    def get_noisyobs(self, data: dict, return_type: str = "r6d") -> torch.Tensor:
        """
    ...
    def normalize_body_pose_r6d(self, body_pose_r6d: torch.Tensor) -> torch.Tensor:
    ...
    def get_local_pos(self, betas: torch.Tensor) -> torch.Tensor:
        skeleton = self.smplx_model.get_skeleton(betas)[..., :22, :]  # (B, L, 22, 3)
        local_skeleton = skeleton - skeleton[:, :, self.parents_tensor]
        local_skeleton = torch.cat([skeleton[:, :, :1], local_skeleton[:, :, 1:]], dim=2)
    ...
    def encode_translw(self, inputs: dict) -> torch.Tensor:
        """
        definition: {
                body_pose_r6d,  # (B, L, (J-1)*6) -> 0:126
                betas, # (B, L, 10) -> 126:136
