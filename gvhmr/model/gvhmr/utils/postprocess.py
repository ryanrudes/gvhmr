...
from gvhmr.utils.geo_transform import apply_T_on_points, transform_mat
from gvhmr.utils.ik.ccd_ik import CCD_IK
from gvhmr.utils.net_utils import gaussian_smooth


@autocast("cuda", enabled=False)
def pp_static_joint(outputs: dict, endecoder: EnDecoder) -> torch.Tensor:
    # Global FK
...
@autocast("cuda", enabled=False)
def pp_static_joint_cam(outputs: dict, endecoder: EnDecoder) -> torch.Tensor:
    ...
    # Overwrite results: subtracting pred_disp[i-1] from every frame >= i (for all i)
    # is equivalent to subtracting the running cumulative sum from each frame. O(L) not O(L^2).
    disp_cumsum = pred_disp.cumsum(dim=1)  # (B, L-1, 3)
    post_w_transl[:, 1:] -= disp_cumsum
    post_w_j3d[:, 1:] -= disp_cumsum[:, :, None]
...
@autocast("cuda", enabled=False)
def process_ik(outputs: dict, endecoder: EnDecoder) -> torch.Tensor:
